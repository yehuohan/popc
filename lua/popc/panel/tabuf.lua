...
--- @class TabContext
--- @field _buf BufID[] Buffers scoped under tabpage
--- @field _name string Tabpage name

--- @class BufContext
--- @field _cnt integer _cnt = 0 means buffer is closed but not wiped out
...
function M.buf_num(tid)
    return tabctx[tid] and #tabctx[tid]._buf or 0
end

--- Add a tabpage
--- @param tid TabID
function M.add_tab(tid)
    tabctx[tid] = { _buf = {}, _name = '' }
end
...
function M.del_tab(tid)
    ...
    for _, bid in ipairs(tabctx[tid]._buf) do
        bufctx[bid]._cnt = math.max(bufctx[bid]._cnt - 1, 0)
    end
    tabctx[tid] = nil
end
...
function M.add_buf(tid, bid)
    ...
    if vim.list_contains(tab._buf, bid) then
        log('switch buffer: tid = %d, tnr = %d, bid = %d, name = %s', tid, tnr, bid, info.name)
    else
        table.insert(tab._buf, bid)
        if bufctx[bid] then
            bufctx[bid]._cnt = bufctx[bid]._cnt + 1
        else
            bufctx[bid] = { _cnt = 1 }
        end
        log('append buffer: tid = %d, tnr = %d, bid = %d, name = %s', tid, tnr, bid, info.name)
    end
    tab._name = info.name == '' and tostring(bid) .. 'NoName' or vim.fs.basename(info.name)
end
...
function M.del_buf(tid, bid)
    ...
    local idx = list_index(tabctx[tid]._buf, bid)
    if idx then
        table.remove(tabctx[tid]._buf, idx)
        bufctx[bid]._cnt = math.max(bufctx[bid]._cnt - 1, 0)
    end
end
...
function M.get_tabs()
    ...
    for _, tid in ipairs(api.nvim_list_tabpages()) do
        res[#res + 1] = {
            tid = tid,
            name = tabctx[tid]._name .. num2str(M.buf_num(tid)),
            current = tid == cur_tid,
            modified = #M.modified_bufs(tid) > 0,
        }
    end
    return res
end

--- Get tabpage's buffers information
--- @param tid TabID|nil nil for nvim_get_current_tabpage()
function M.get_bufs(tid)
    tid = tid or api.nvim_get_current_tabpage()
    if not tabctx[tid] then
        return {}
    end
...
function M.buf_callback(args)
    ...
    if args.event == 'BufEnter' then
        ...
        M.add_buf(tid, api.nvim_get_current_buf())
    elseif args.event == 'BufWipeout' then
        ...
        if bufctx[bid] then
            ...
            bufctx[bid] = nil
            local tnr = api.nvim_tabpage_get_number(tid)
            log('wipeout buffer: tid = %d, tnr = %d, bid = %d, afile = %s', tid, tnr, bid, args.file)
        end
